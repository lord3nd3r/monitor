	...
	with _DB_LOCK:
		...
		try:
			...
			bottom = conn.execute(
				...
				FROM stats
				WHERE channel = ?
				ORDER BY messages ASC, nick ASC
				LIMIT 10
				""",
				(channel.lower(),),
			).fetchall()
...
def userstats(bot, trigger):
	...
	bot.say(
		f"{nick} in {channel}: messages={messages}, "
		f"first_seen={_format_ts(first_seen)}, last_seen={_format_ts(last_seen)}"
	)
